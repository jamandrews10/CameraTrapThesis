...
#ADDED TO GET IMPORTS TO WORK
# sys.path.insert(0,"c:/megadetector/test_env/")
...
from Program.databaseClass import *
...
def main():
    ...
    image_file='c:\\dataset\\dog'
    output_file='c:\\megadetector\\dog_output.json'
    recursive=True
    ...
    detector_file = try_download_known_detector(detector_file)
    ...
    if output_relative_filenames:
        assert os.path.isdir(image_file), \
            f'Could not find folder {image_file}, must supply a folder when ' + \
    ...
    if os.path.isdir(image_file):
        image_file_names = path_utils.find_images(image_file, recursive)
        if len(image_file_names) > 0:
            print('{} image files found in the input directory'.format(len(image_file_names)))                        
        else:
            if (recursive):
                print('No image files found in directory {}, exiting'.format(image_file))
            else:
    ...
    # A json list of image paths
        ...
        print('Loaded {} image filenames from list file {}'.format(
            len(image_file_names),image_file))
        
    # A single image file
    elif os.path.isfile(image_file) and path_utils.is_image_file(image_file):
    ...
    if checkpoint_frequency != -1:
        ...
        if (checkpoint_path is not None) and (not allow_checkpoint_overwrite) \
